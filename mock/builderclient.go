...
// CombinedProvider combines multiple provider interfaces for integration testing.
// This allows testing scenarios where a single provider needs to support both
// v2 (SubmitBlindedProposal) and v1 (UnblindProposal) operations.
type CombinedProvider struct {
	SubmitProvider  *CustomSubmitBlindedProposalProvider
	UnblindProvider *CustomUnblindedProposalProvider
}

// Name returns the name of the combined provider.
func (c *CombinedProvider) Name() string {
	return "mock-combined"
}

// Address returns the address of the combined provider.
func (c *CombinedProvider) Address() string {
	return "mock-combined:12345"
}

// Pubkey returns the public key of the combined provider.
func (c *CombinedProvider) Pubkey() *phase0.BLSPubKey {
	return nil
}

// BuilderBid implements the BuilderBidProvider interface.
func (c *CombinedProvider) BuilderBid(_ context.Context, _ *builderapi.BuilderBidOpts) (*builderapi.Response[*builderspec.VersionedSignedBuilderBid], error) {
	return nil, nil
}

// SubmitBlindedProposal delegates to the SubmitProvider.
func (c *CombinedProvider) SubmitBlindedProposal(ctx context.Context, opts *builderapi.SubmitBlindedProposalOpts) error {
	return c.SubmitProvider.SubmitBlindedProposal(ctx, opts)
}
