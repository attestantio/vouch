...
func (s *Service) SubmitVersionedAttestations(ctx context.Context, opts *api.SubmitAttestationsOpts) error {
	ctx, span := otel.Tracer("attestantio.vouch.service.submitter.multinode").Start(ctx, "SubmitVersionedAttestations", trace.WithAttributes(
	...
	w.L.Lock()
	for name, submitter := range s.versionedAttestationsSubmitters {
		go s.submitVersionedAttestations(ctx, sem, w, submissionCompleted, name, attestations, submitter)
	}
...
// submitVersionedAttestations carries out the internal work of submitting attestations.
// skipcq: RVV-B0001
func (s *Service) submitVersionedAttestations(ctx context.Context,
	...
	_, err = util.Scatter(len(attestations), int(s.processConcurrency), func(offset int, entries int, _ *sync.RWMutex) (interface{}, error) {
		return nil, submitter.SubmitVersionedAttestations(ctx, &api.SubmitAttestationsOpts{
			Attestations: attestations[offset : offset+entries],
		})
	})
	if err != nil {
		err = s.handleVersionedAttestationsError(ctx, submitter, err)
	}

	s.clientMonitor.ClientOperation(address, "submit versioned attestations", err == nil, time.Since(started))
	if err != nil {
		log.Warn().Err(err).Msg("Failed to submit versioned attestations")
		return
	}

	submissionCompleted.Store(true)
	w.Signal()
	log.Trace().Msg("Submitted versioned attestations")
}

func (s *Service) handleVersionedAttestationsError(ctx context.Context,
	submitter eth2client.VersionedAttestationsSubmitter,
	err error,
) error {
