...
// Package first is a strategy that obtains beacon block headers
...
type parameters struct {
	...
	beaconBlockHeadersProviders map[string]consensusclient.BeaconBlockHeadersProvider
...
func WithBeaconBlockHeadersProviders(providers map[string]consensusclient.BeaconBlockHeadersProvider) Parameter {
	return parameterFunc(func(p *parameters) {
		p.beaconBlockHeadersProviders = providers
	})
}
...
func parseAndCheckParameters(params ...Parameter) (*parameters, error) {
	...
	if len(parameters.beaconBlockHeadersProviders) == 0 {
		return nil, errors.New("no beacon block headers providers specified")
	}
