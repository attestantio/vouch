...
// WithDeadline sets the deadline for fetching builder bids.
func WithDeadline(deadline time.Duration) Parameter {
	return parameterFunc(func(p *parameters) {
		p.deadline = deadline
	})
}

// WithBidGap sets the gap between fetching builder bids.
func WithBidGap(bidGap time.Duration) Parameter {
	return parameterFunc(func(p *parameters) {
		p.bidGap = bidGap
	})
}
...
func parseAndCheckParameters(params ...Parameter) (*parameters, error) {
	...
	if parameters.deadline == 0 {
		return nil, errors.New("no deadline specified")
	}
	if parameters.bidGap == 0 {
		return nil, errors.New("no bid gap specified")
	}
